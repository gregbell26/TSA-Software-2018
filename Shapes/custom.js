var createVertices = [];
var createFaces = [];
function newCustom(x, y, z, posX, posY, posZ, newColor, borderColor){
    // GEOMETRY
    var newGeometry = new THREE.Geometry(1, 1, 1);

        for (var i = 0; i < createVertices.length; i++){
            newGeometry.vertices.push(createVertices[i]);
        }

        for (var i = 0; i < createFaces.length; i++){
