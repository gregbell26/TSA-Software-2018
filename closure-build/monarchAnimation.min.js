...
$(document).on("keydown",function(a){1!=inAnimationWindow||settingsOpen||animationRunning&&lockCamera||("-"==a.key?zoomCameraM(settings.camera.zoomAmount):"="==a.key&&zoomCameraM(1/settings.camera.zoomAmount))});
$(document).on("wheel",function(a){mobile&&"init"!==activeMenu||(console.log(a.originalEvent.deltaY),1!=inAnimationWindow||settingsOpen||animationRunning&&lockCamera||(0<a.originalEvent.deltaY?zoomCameraM(settings.camera.zoomAmount/1.16667):0>a.originalEvent.deltaY&&zoomCameraM(1/(settings.camera.zoomAmount/1.16667))))});$(document).on("mouseup",function(a){mouseOnTimeline=mouseDown=!1});var timelinePosition=0;
$(document).ready(function(){$(document).on("mousemove",function(a){inAnimationWindow=300<=a.pageX&&50<=a.pageY&&a.pageY<window.innerHeight-150?1:0;!mouseDown||1!=inAnimationWindow||settingsOpen||animationRunning&&lockCamera||(rotateCamera(settings.camera.mouseSensitivity*(a.pageX-xStart)/100,settings.camera.mouseSensitivity*(a.pageY-yStart)/100),300<=a.pageX&&50<=a.pageY&&a.pageY<window.innerHeight-70&&(xStart=a.pageX,yStart=a.pageY));mouseOnTimeline&&timelineScrub(a.pageX)})});
$(document).ready(function(){$(document).on("touchmove",function(a){inAnimationWindow=50<=a.touches[0].pageY&&a.touches[0].pageY<window.innerHeight-150?1:0;"init"!==activeMenu||1!=inAnimationWindow||settingsOpen||animationRunning&&lockCamera||(rotateCamera(settings.camera.mouseSensitivity*(a.touches[0].pageX-xStart)/100,settings.camera.mouseSensitivity*(a.touches[0].pageY-yStart)/100),50<=a.touches[0].pageY&&a.touches[0].pageY<window.innerHeight-150&&(xStart=a.touches[0].pageX,yStart=a.touches[0].pageY));
mouseOnTimeline&&timelineScrub(a.touches[0].pageX)})});
function zoomCameraM(a){console.log(a);zoom=Math.pow(Math.pow(xPosition,2)+Math.pow(yPosition,2)+Math.pow(zPosition,2),.5);if(zoom*a>1*Math.pow(10,100)||zoom*a<1*Math.pow(10,-100))zPosition=yPosition=xPosition=10,showPopUp("popUp_error_body","You dun messed up","You have zoomed to infinity and entered easter egg mode, where css styling degrades into madness and everything is pain. Remember to save your changes before refreshing the page to end this madness.",-1),stylesheetLoader("memeMode");else{zoom*=
a;zoomZ=Math.cos(Math.asin(yPosition/zoom));var b,c;0!==xPosition?b=Math.atan(zPosition/xPosition):0<zPosition?b=Math.PI:0>zPosition&&(b=-Math.PI);0>xPosition&&(0>b?b+=Math.PI:0<b&&(b-=Math.PI));0!==xPosition||0!==zPosition?c=Math.atan(yPosition/Math.pow(Math.pow(xPosition,2)+Math.pow(zPosition,2),.5)):0<yPosition?c=Math.PI:0>yPosition&&(c=-Math.PI);yPosition=zoom*Math.sin(c);xPosition=zoom*Math.cos(Math.asin(yPosition/zoom))*Math.cos(b);zPosition=zoom*Math.cos(Math.asin(yPosition/zoom))*Math.sin(b);
getId("camera_x").value=xPosition;getId("camera_y").value=yPosition;getId("camera_z").value=zPosition;isNaN(yPosition)&&(yPosition=0);isNaN(xPosition)&&(xPosition=0);isNaN(zPosition)&&(zPosition=0)}}
...
CloudStorage.prototype.downloadSave=function(a){this.saveDataRef.doc(a).get().then(function(b){if(b.exists){saveEngine.cloudStorage.downloadData=b.data();alert("HOE");saveEngine.stagedScene=conversion.convertJSONToScene(JSON.parse(b.data().scene));saveEngine.stagedKeyframes=JSON.parse(b.data().keyframes);saveEngine.localFileName=b.data().name;saveEngine.localFileID=a;saveEngine.localStore.saveToStorage(saveEngine.localFileID,saveEngine.localFileName);saveEngine.localStore.saveToStorage(saveEngine.getKeyName("scene"),
